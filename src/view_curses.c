...
const s_helpitem thelp[] = {
  ...
  { descr: "Toggle showing this help", k1: "          ?", k2: "h", k3:"H" },
                                // padded to match <page-down>
...
static inline void draw_vscroll(int xpos, int from, int to, int items, int pos)
{
    ...
    int is_unicode = unicode && has_unicode;
    attron(A_REVERSE);
    if (from == to) {
        if (is_unicode) {
        ...
        int drscale = is_unicode ? 8 : 1;  // draw scale
        ...
        int begpos =
            (pos * (linecnt * drscale - scrolsz)) / poitems +
            (from + 1) * drscale;
        ...
        for (i = from; i <= to; i++) {
            if (i == from || i == to) {
                attron(A_REVERSE);
                mvprintw(i, xpos,
                    "%s", (is_unicode ? scroll_u : scroll_a)[i == from ? 1 : 2]);
                attroff(A_REVERSE);
            }
            if (i != from && i != to) {
                if (is_unicode) {
                    if (items <= to - from + 1)
                        ...
                        if (i == begpos / 8)
                            mvprintw(i, xpos, "%s",
                                     scroll_u[4 + 7 - begpos % 8]);
                        if (i == endpos / 8 && i != begpos / 8) {
                            attron(A_REVERSE);
                            mvprintw(i, xpos, "%s",
                                     scroll_u[4 + 7 - endpos % 8]);
                            attroff(A_REVERSE);
                        }
...
static inline void view_help(void)
{
    ...
    for (p = thelp, i = 1; i < hh - 1; i++, p++)
        mvwprintw(whelp, i, 0, " %-*.*s %-*.*s %-*.*s - %-*.*s ", a, a,
                  p->k1 ? p->k1 : "", b, b, p->k2 ? p->k2 : "", c, c,
                  p->k3 ? p->k3 : "", d, d, p->descr);
    mvwprintw(whelp, hh - 1, 0, "%s", (has_unicode && unicode) ? "─" : "_");
    for (i = 1; i < hw; i++)
        wprintw(whelp, "%s", (has_unicode && unicode) ? "─" : "_");
}

static inline void view_curses(struct xxxid_stats_arr *cs,
                               struct xxxid_stats_arr *ps,
                               struct act_stats *act, int roll)
{
    ...
    diff_len =
        create_diff(cs, ps, time_s, filter_view,
                    (has_unicode
                     && unicode) ? gr_width * 2 : gr_width, &dispcount);
    ...
    if (maxy < 10
        || maxx <
        (int) strlen(HEADER1_FORMAT) + 2 * (7 - 5 + 3 - 2 +
                                            (!config.f.hidegraph ? gr_width_h +
                                             1 : 0) - 2)) {
        int size_off;
        ...
        size_off =
            7 - 5 + 3 - 2 + (!config.f.hidegraph ? gr_width_h + 1 : 0) - 2;
        ...
        if (!config.f.hidegraph)
            while (gr_width_h < gr_width
                   && maxx >=
                   (int) strlen(head1row_format) + 4 * (size_off + 1)) {
                size_off++;
                gr_width_h++;
            }
    }

    for (i = 0; i < ((has_unicode && unicode) ? gr_width_h * 2 : gr_width_h);
         i++) {
    ...
    for (j = 0; j < gr_width_h; j++) {
        if (has_unicode && unicode) {
            strcat(pg_t_r,
                   br_graph[value2scale(hist_t_r[j * 2], mx_t_r)][value2scale
                                                                  (hist_t_r
                                                                   [j * 2 + 1],
                                                                   mx_t_r)]);
            strcat(pg_t_w,
                   br_graph[value2scale(hist_t_w[j * 2], mx_t_w)][value2scale
                                                                  (hist_t_w
                                                                   [j * 2 + 1],
                                                                   mx_t_w)]);
            strcat(pg_a_r,
                   br_graph[value2scale(hist_a_r[j * 2], mx_a_r)][value2scale
                                                                  (hist_a_r
                                                                   [j * 2 + 1],
                                                                   mx_a_r)]);
            strcat(pg_a_w,
                   br_graph[value2scale(hist_a_w[j * 2], mx_a_w)][value2scale
                                                                  (hist_a_w
                                                                   [j * 2 + 1],
                                                                   mx_a_w)]);
        } else {
    ...
    if (head1row) {
        ...
        if (!in_ionice && !in_filter) {
            ...
            mvprintw(0, 0, head1row_format, total_read, str_read,
                     ...
                     !config.f.hidegraph ? pg_t_w : "", total_a_read,
                     str_a_read, !config.f.hidegraph ? pg_a_r : "",
                     total_a_write, str_a_write,
        ...
        if (!in_ionice && !in_filter) {
            ...
            mvprintw(1, 0, HEADER2_FORMAT, total_a_read, str_a_read,
                     !config.f.hidegraph ? pg_a_r : "", total_a_write,
                     str_a_write, !config.f.hidegraph ? pg_a_w : "");
            show = FALSE;
        }
    }
    ...
    if (dontrefresh
        && (maxx - maxcmdline +
            (config.f.hidecmd ? 0 : strlen(COLUMN_L(0)) + 1) < (size_t) maxx)) {
        size_t xpos = maxx - strlen("[freezed]");

        // don't step on column descriptions
        if (xpos <
            maxx - maxcmdline +
              (config.f.hidecmd ? 0 : strlen(COLUMN_L(0)) + 1)) {
            xpos =
                maxx - maxcmdline +
                (config.f.hidecmd ? 0 : strlen(COLUMN_L(0)) + 1);
        }
    ...
    iotop_sort_cb(NULL,
                  (void *) (long) ((has_unicode && unicode) ? gr_width *
                                   2 : gr_width));
    ...
    for (i = 0; cs->sor && i < diff_len; i++) {
        ...
        for (k = -1; ms->threads && ms->threads->arr && k < ms->threads->length;
             k++) {
            if (k < 0)
                s = ms;
            else {
        ...
        for (k = -1;
             k <
             (config.f.processes ? 0 : (ms->threads ? ms->threads->length : 0));
             k++) {
            if (k < 0)
                s = ms;
            else {
            ...
            // apply filters
            if (filter_view
                (s, (has_unicode && unicode) ? gr_width * 2 : gr_width))
                continue;
            if (skip) {
                skip--;
                continue;
            ...
            if (pwt)
                free(pwt);
            cmdt =
                esc_low_ascii(config.f.fullcmdline ? s->cmdline2 : s->cmdline1);
            ...
            if (!config.f.hidecmd) {
                const char *ss =
                    (has_unicode && unicode) ? th_lines_u[0] : th_lines_a[0];

                if (ms->threads) {
                    if (config.f.processes) {
                        if (k == -1 && ms->threads->length)
                            ss = (has_unicode && unicode) ? th_lines_u[1] : th_lines_a[1];
                    } else if (th_first_id != th_last_id) {
                        if (k == th_first_id)
  ...
  donedraw:
    ...
    if (in_ionice) {
        ...
        if (strlen(ionice_id)) {
            ...
            printw(" ");
            attron(A_REVERSE);
            printw("[use 0-9/bksp for %s, tab and arrows for prio]",
                   COLUMN_NAME(0));
            attroff(A_REVERSE);
        } else {
            if (ionice_pos == -1 || ionice_pos_data == NULL
                || ionice_pos_data->exited)
                printw(" (select %s by arrows or enter by 0-9/bksp)",
                       COLUMN_NAME(0));
            else {
                attron(A_BOLD);
                if (ionice_col == 0)
    ...
    if (in_filter) {
        ...
        if (params.user_id < 0)
            ...
            printw("%d [%s]", params.user_id, pwd
                   && pwd->pw_name ? pwd->pw_name : "n/a");
        }
        attroff(A_BOLD);
        printw(" Change to: ");
        attron(A_BOLD);
...
static inline int curses_key(int ch)
{
    ...
    case 'R':
        config.f.sort_order =
            (config.f.sort_order == SORT_ASC) ? SORT_DESC : SORT_ASC;
        break;
    case KEY_HOME:
        scrollpos = 0;
        break;
        ...
        if (in_ionice) {
            if (strlen(ionice_id)) {
                ...
                case 0:
                    ...
                    if (ionice_pos == -1)
                        ionice_pos = ionice_line + 2;
                    else if (ionice_pos + 1 < lastvisible)
                        ionice_pos++;
                    break;
                case 1:
                    ionice_class--;
    ...
    case '0' ... '9':
        ...
        if (in_filter) {
            ...
            if (idlen <
                (filter_col ? sizeof filter_pid : sizeof filter_uid) - 1) {
