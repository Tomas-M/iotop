                    ...
                    __u8 genl_cmd, __u16 nla_type, void *nla_data, int nla_len)
{
    ...
    while ((r =
            sendto(sock_fd, buf, buflen, 0, (struct sockaddr *) &nladdr,
...
inline int get_family_id(int sock_fd)
{
    ...
    if (send_cmd
        (sock_fd, GENL_ID_CTRL, getpid(), CTRL_CMD_GETFAMILY,
         CTRL_ATTR_FAMILY_NAME, (void *) name, strlen(TASKSTATS_GENL_NAME) + 1))
        return 0;

    rep_len = recv(sock_fd, &answ, sizeof answ, 0);
    if (answ.n.nlmsg_type == NLMSG_ERROR || (rep_len < 0)
        || !NLMSG_OK((&answ.n), rep_len))
        return 0;
...
inline int nl_xxxid_info(pid_t tid, pid_t pid, struct xxxid_stats *stats)
{
    ...
    if (send_cmd
        (nl_sock, nl_fam_id, tid, TASKSTATS_CMD_GET, TASKSTATS_CMD_ATTR_PID,
         &tid, sizeof tid)) {
...
static void pid_cb(pid_t pid, pid_t tid, struct xxxid_stats_arr *a,
