...
inline int system_checks(void)
{
    ...
    if (geteuid() == 0)
        ...
        struct __user_cap_header_struct caphdr = {.version =
                _LINUX_CAPABILITY_VERSION_3,.pid = 0, };
        struct __user_cap_data_struct cap[_LINUX_CAPABILITY_U32S_3];

        if (!syscall(SYS_capget, &caphdr, cap))
            if (cap[CAP_TO_INDEX(CAP_NET_ADMIN)].
                effective & CAP_TO_MASK(CAP_NET_ADMIN))
                root_or_netadm = 1;
    }
    if (!root_or_netadm) {
        printf
    ...
    else if (S_IFREG == (s.st_mode & S_IFMT))
        io_accounting = 1;
