...
inline void calc_a_total(struct act_stats *act, double *read, double *write,
                         double time_s)
{
    *read = *write = 0;
...
inline int create_diff(struct xxxid_stats_arr *cs, struct xxxid_stats_arr *ps,
                       double time_s, filter_callback_w cb, int width, int *cnt)
{
    int n = 0;
    ...
    for (n = 0; cs->arr && n < cs->length; n++) {
        ...
        if (!p) {               // new process or task
            ...
            maxpidlen =
                maxpidlen < (int) strlen(temp) ? (int) strlen(temp) : maxpidlen;
            continue;
        }
        // round robin value
        c->blkio_val =
        ...
        if (ps->arr[n]->exited || !arr_find(cs, ps->arr[n]->tid)) {
            ...
            if (p) {
                *p = *ps->arr[n];       // WARNING - all dynamic data inside should always be initialized below
                ...
                memmove(p->iohist + 1, p->iohist,
                        sizeof p->iohist - sizeof *p->iohist);
                p->iohist[0] = 0;
...
inline void humanize_val(double *value, char *str, int allow_accum)
{
    ...
    snprintf(str, 4, "%c%s", u[p], config.f.accumulated
             && allow_accum ? "  " : "/s");
}
...
inline int iotop_sort_cb(const void *a, const void *b)
{
    ...
    case SORT_BY_COMMAND:
        res =
            strcmp(config.f.fullcmdline ? pa->cmdline2 : pa->cmdline1,
                   config.f.fullcmdline ? pb->cmdline2 : pb->cmdline1);
        break;
    case SORT_BY_PID:
        res = pa->tid - pb->tid;
        break;
    ...
    case SORT_BY_READ:
        if (config.f.accumulated)
            res =
                pa->read_val_acc > pb->read_val_acc ? 1 : pa->read_val_acc <
                pb->read_val_acc ? -1 : 0;
        else
            res =
                pa->read_val > pb->read_val ? 1 : pa->read_val <
                pb->read_val ? -1 : 0;
        break;
    case SORT_BY_WRITE:
        if (config.f.accumulated)
            res =
                pa->write_val_acc > pb->write_val_acc ? 1 : pa->write_val_acc <
                pb->write_val_acc ? -1 : 0;
        else
            res =
                pa->write_val > pb->write_val ? 1 : pa->write_val <
                pb->write_val ? -1 : 0;
        break;
    case SORT_BY_SWAPIN:
        res =
            pa->swapin_val > pb->swapin_val ? 1 : pa->swapin_val <
            pb->swapin_val ? -1 : 0;
        break;
    case SORT_BY_IO:
        res =
            pa->blkio_val > pb->blkio_val ? 1 : pa->blkio_val <
            pb->blkio_val ? -1 : 0;
        break;
    }
    res *= order;
